...
$(function() {
    ...
    $("#delCndBtn").click(function () {
        ...
        if ($li.size() == 0) {
            $.qicTips({message:'请选择角色名', level:2, target:this, mleft:0, mtop:-10});
            return;
        }
        var cndId = $li.attr("id").substring("ut_".length);
        if (cndId == -999) {
        ...
        $("#ut_ul_list > li").each(function(){
           // alert($(this).attr('roleid'))
            if($(this).attr('roleid')==undefined){
               $.qicTips({message:"请双击编辑当前角色", level:2, target:'#ut_ul_list', mleft:0, mtop:-60});
                input.focus();
                return;
            }
        })
 ...
 /*   if(li.attr("id") != ("ut_"+$("#curRoleId").val())){
    ...
    input.blur(function (event){//失去焦点
        ...
        if(inputtext.length == 0){
            $.qicTips({message:"角色名不能为空", level:2, target:'#ut_ul_list', mleft:0, mtop:-60});
            input.focus();
            return ;
               ...
               // data:{"type":typeVal},
                ...
                success:function (data) {
                    ...
                    if (data.op) {
                        ...
                        setTimeout(function() {
                                window.location.replace("/RoleInfos/roleList?tab=0&scroll=bottom&roleId="+data.id);
                            },
                            400)
