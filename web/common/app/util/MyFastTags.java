    ...
    public static void _emVFC(Map<?, ?> args, Closure body, PrintWriter out, GroovyTemplate.ExecutableTemplate template, int fromLine ) {
        ...
        if (v == null) {
            if (d == null) {
                ...
                objFormateC(out, d, f, u, c,l);
            }
            return;
        }
        objFormateC(out, v, f, u, c,l);
    }

    //加入了样式控制
    private static void objFormateC(PrintWriter out, Object arg, String f, String u,Boolean c,Boolean l) {
        String startLabel = "<Strong>";
        String endLabel ="</Strong>";
        if (arg instanceof Number) { //处理数字
            Number val = (Number) arg;
            if(((Number) val).floatValue()>=0){
                if(c){
                    startLabel = "<Strong style='color:red'>";
                    endLabel = "</strong>";
                }
            }else {
                if(c){
                    startLabel = "<Strong style='color:green'>";
                    endLabel = "</strong>";
                }
            }
            if (f == null) {
