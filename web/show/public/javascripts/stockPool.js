...
function updateQuotationInfo(stockInfo){
    ...
    for(var i=0;i<data.length;i++){
        $('.tbl_common tr').each(function () {
            if($(this).children('td').eq(0).children('a').html()==data[i].Code){
               // $(this).children('td').eq(1).html(stockInfo.name);
                $(this).children('td').eq(2).html("<strong>"+data[i].Price.toFixed(2)+"</strong>");
                $(this).children('td').eq(3).html("<strong>"+data[i].Change.toFixed(2)+"</strong>");
                $(this).children('td').eq(4).html("<strong>"+data[i].ChangeRate.toFixed(2)+"</strong>");
                $(this).children('td').eq(5).html("<strong>"+data[i].TotalVol.toFixed(2)+"</strong>");
                $(this).children('td').eq(6).html("<strong>"+data[i].TotalAmount.toFixed(2)+"</strong>");
                $(this).children('td').eq(7).html("<strong>"+data[i].Bid1.toFixed(2) + "</strong>/<strong>" + data[i].Ask1.toFixed(2)+"</strong>");
                $(this).children('td').eq(8).html("<strong>"+data[i].BidSize1.toFixed(2) + "</strong>万/<strong>" + data[i].AskSize1.toFixed(2)+"</strong>万");
                $(this).children('td').eq(9).html("<strong>"+data[i].Pre.toFixed(2) + "</strong>/<strong>" + data[i].Open.toFixed(2)+"</strong>");
                $(this).children('td').eq(10).html("<strong>"+data[i].High.toFixed(2) + "</strong>/<strong>" + data[i].Low.toFixed(2)+"</strong>");
            }
        });
    }
}
...
function combinQuotationInfo(id){
    ...
    $('#combindata_'+id+' tr').each(function () {
        scode = $(this).children('td').eq(0).children('a').html()
